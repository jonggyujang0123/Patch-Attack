...
class patch_util():
    ...
    def get_patch(self, img, pat_ind = None):
        ...
        img_masked = img_masked * self.mask_to_extended_patch - 999 * (1-self.mask_to_extended_patch)
        img_masked = img_masked[img_masked > -3].reshape([img_masked.shape[0],-1])
        # (bs, len_conditional)

        return patch, img_masked, pat_ind
