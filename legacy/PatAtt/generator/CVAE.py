...
def train(wandb, args, cfg, model):
    ...
    for epoch in tqdm(range(start_epoch, cfg.epochs)):
        ...
        for data in tepoch:
            ...
            patch, cond_img, pat_ind,_ = patch_u.get_patch(inputs)
            recon_batch, mu, log_var = model.forward(patch, cond_img)
    ...
    if dataset == None:
        ...
        if args.local_rank in [-1,0]:
            with torch.no_grad():
                for pat_ind_const in range(patch_u.num_patches):
                    ...
                    new_patch = model.decode(fixed_noise[:, pat_ind_const, :], cond_img)
        ...
        if args.local_rank in [-1,0]:
            with torch.no_grad():
                for pat_ind_const in range(patch_u.num_patches):
                    pat_ind = torch.tensor(np.ones(shape=(cfg.test_batch_size,), dtype=int)).to(cfg.device) * pat_ind_const
                    patch, cond_img, pat_ind,_ = patch_u.get_patch(inputs, pat_ind = pat_ind)
                    embedding, _ = model.encode(patch, cond_img)
                    if pat_ind_const % 10 == 0:
                        print(embedding.abs().mean().item())
                    #new_patch = model.decode(embedding, cond_img)
                    new_patch, _, _ = model(patch, cond_img)
                    inputs = patch_u.concat_patch(inputs, new_patch, pat_ind)
                    samples.append(inputs)
            gif = rearrange(torch.cat(samples),
                    '(t b1 b2 ) c h w -> t (b1 h) (b2 w) c', b1=4, b2=4
