...
class Generator(nn.Module):
    def __init__(self, 
        ...
        self.latent_layer = nn.Sequential(
                nn.Linear(latent_size + n_classes + len_code, n_gf * 2**(levels) * self.init_size**2),
                nn.BatchNorm1d(n_gf * 2**(levels) * self.init_size**2),
                nn.LeakyReLU(0.2, inplace=True),
                nn.Linear(n_gf * 2**(levels) * self.init_size**2, n_gf * 2**(levels) * self.init_size**2),
                nn.BatchNorm1d(n_gf * 2**(levels) * self.init_size**2),
                nn.LeakyReLU(0.2, inplace=True),
...
#        return self.dropout(torch.cat([x, self.pos_emb2D.to(x.device).tile([x.shape[0], 1, 1, 1])], dim=1))
#        return self.dropout(x * self.pos_emb2D.to(x.device))
